...
export const FileInput = ({ imageFiles, audioFiles, onFilesAdded, onFileRemoved }: FileInputProps) => {
    ...
    const onDrop = useCallback((acceptedFiles: File[], fileRejections: FileRejection[]) => {
        ...
        acceptedFiles.forEach((file: File) => {
            const isDuplicate = !(
                imageFiles.filter(existingImageFile => existingImageFile.name === file.name).length === 0 &&
                audioFiles.filter(existingAudioFile => existingAudioFile.name === file.name).length === 0
            )
            if (!isDuplicate) {
